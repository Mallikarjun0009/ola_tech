...
# web_ip_update_gestioip_snmp.pl Version 3.4.2

# script para actualizar la BBDD del sistema GestioIP via SNMP queries


use lib qw(/var/www/gestioip/modules /usr/share/gestioip/bin/web/include);
use GestioIP;
use Usage;
use strict;
use Getopt::Long;
...
#use Cwd qw();
#my $path = Cwd::cwd();
#require "/usr/share/gestioip/bin/web/include/usage_func.pl";
#print STDERR "TEST PATH $path\n";
#require "$path/include/usage_func.pl";
...
#	$pm = new Parallel::ForkManager($MAX_PROCESSES);
	...
	$pm->run_on_finish(
		sub { my ($pid, $exit_code, $ident) = @_;
			$res_sub{$pid}=$exit_code if $exit_code < 2;
		}
	);
	$pm->run_on_start(
		sub { my ($pid,$ident)=@_;
...
#					mod_ini_stat("$new_host_count");
				...
				if ( ! $node_id && $device_name ) {
					$device_name =~ s/\s/_/g;
					$node_id = insert_ip_mod("$client_id","$i","$device_name","","$red_loc_id","n","$device_cat","","-1","$mydatetime","$red_num","-1","$ip_version");
					$new_host = "1";
					$pm->finish(0) if ! $node_id;
					print LOG "$lang_vars{host_anadido_message}: $device_name";
					$exit++;
				} elsif ( ! $node_id && $device_name_dns ) {
					$node_id = insert_ip_mod("$client_id","$i","$device_name_dns","","$red_loc_id","n","$device_cat","","-1","$mydatetime","$red_num","-1","$ip_version");
					$new_host = "1";
					$pm->finish(0) if ! $node_id;
