...
my @all_db_networks = get_all_networks("$client_id","v4","no_rootnet") if $ipv4;
...
if (( $route_proto =~ /local/ && $local_routes ) || ( $route_proto =~ /netmgmt/ && $static_routes ) || ( $route_proto =~ /other/ && $other_routes ) || ( $route_proto =~ /ospf/ && $ospf_routes ) || ( $route_proto =~ /^rip$/ && $rip_routes ) || ( $route_proto =~ /isIs/ && $isis_routes ) || ( $route_proto =~ /ciscoEigrp/ && $eigrp_routes )) {
        ...
        $vars = new SNMP::VarList(['inetCidrRouteProto']
                    );
...
#       if ($session->{ErrorStr} && ! $route_dests_cidr_ipv6[0] ) {
        ...
        while (!$session->{ErrorStr} and
               $$vars[0]->tag eq "inetCidrRouteProto"){
            ($route_dest) = $session->getnext($vars);

            my $iid=$$vars[0]->iid;

#            print "TEST TEST inetCidrRouteProto: $iid\n";
            # 1.4.192.168.1.0.24.3.0.0.4.1.4.0.0.0.0

            if ( $iid !~ /^((\d{1,3}\.){16}\d{1,3})$/ ) {
                next;
            }
            ...
            # print "TEST: $iid - $route_dest - $ip_mask - $route_mask - $route_proto\n";
	...
	if ( $ipv6 ) {
        ...
        my $vars = new SNMP::VarList(['inetCidrRouteProto']
                    );
	...
	foreach $network_cidr_mask(@route_dests_cidr) {
		...
		if ( $overlap != "2" ) {
			...
			if ( $add_comment == "1" ) {
				$red_num = insert_networks($network,$BM,$comment,$client_id,$set_sync_flag,"v4","$rootnet_val");
			} else {
				$red_num = insert_networks($network,$BM,"",$client_id,$set_sync_flag,"v4","$rootnet_val");
			}
			my $audit_type="17";
			my $audit_class="2";
			my $update_type_audit="3";
	...
	foreach $network_cidr_mask(@route_dests_cidr_ipv6) {
		...
		if ( $overlap != "2" ) {
			...
			print LOG "$network/$BM: ADDED\n";
			if ( $add_comment == "1" ) {
				$red_num = insert_networks($network,$BM,$comment,$client_id,$set_sync_flag,"v6","$rootnet_val");
			} else {
				$red_num = insert_networks($network,$BM,"",$client_id,$set_sync_flag,"v6","$rootnet_val");
			}
...
#sub insert_networks {
#    my ($network,$BM,$comment,$client_id,$set_sync_flag,$ip_version,$rootnet) = @_;
...
#        my $sth = $dbh->prepare("INSERT INTO net (red,bm,descr,loc,vigilada,comentario,categoria,ip_version,client_id,rootnet) VALUES ( \"$network\", \"$BM\", \"\",\"-1\",\"$set_sync_flag\",\"$comment\",\"-1\",\"$ip_version\",\"$client_id\",\"$rootnet\")"
#                                ) or die "Fehler bei insert db: $DBI::errstr\n";
#    $sth->execute() or die "Fehler bei execute db: $DBI::errstr\n";
#
#    $sth = $dbh->prepare("SELECT LAST_INSERT_ID()") or die "Can not execute statement:<p>$DBI::errstr\n";
...
#   my $parent_red_num = get_parent_network("$network", "$BM", $values_redes_rootnet, "$client_id","$ip_version") || "";
    ...
    if ( $parent_red_num ) {
        $sth = $dbh->prepare("INSERT INTO net (red,bm,descr,loc,vigilada,comentario,categoria,client_id,ip_version,rootnet,parent_network_id) VALUES ( \"$network\", \"$BM\", \"\",\"-1\",\"$set_sync_flag\",\"$comment\",\"-1\",\"$client_id\",\"$ip_version\",\"$rootnet\",\"$parent_red_num\")"
                                ) or die "Fehler bei insert db: $DBI::errstr\n";
    } else {
        $sth = $dbh->prepare("INSERT INTO net (red,bm,descr,loc,vigilada,comentario,categoria,client_id,ip_version,rootnet) VALUES ( \"$network\", \"$BM\", \"\",\"-1\",\"$set_sync_flag\",\"$comment\",\"-1\",\"$client_id\",\"$ip_version\",\"$rootnet\")"
                                ) or die "Fehler bei insert db: $DBI::errstr\n";
    }
    $sth->execute() or die "Fehler bei execute db: $DBI::errstr\n";
    ...
    if ( $rootnet == 1 ) {
        ...
        foreach my $child_red_num ( keys %{$child_networks}) {
            ...
            $sth = $dbh->prepare("UPDATE net SET parent_network_id=$qparent_id WHERE red_num=$child_red_num") or die "Can not execute statement:<p>$DBI::errstr";
...
sub get_all_networks {
	my ($client_id,$ip_version, $no_rootnet) = @_;
    my @overlap_redes;
    my $ip_ref;
	my $dbh = $gip->_mysql_connection("$gip_config_file");
	my $sth;
    my $no_rootnet_expr="";
    $no_rootnet="" if ! $no_rootnet;
    $no_rootnet_expr="AND rootnet=0" if $no_rootnet eq "no_rootnet";

	if ( $ip_version eq "v4" ) {
		$sth = $dbh->prepare("SELECT red, BM, rootnet FROM net WHERE client_id = \"$client_id\" AND ip_version = \"$ip_version\" $no_rootnet_expr ORDER BY INET_ATON(red)") or die "Can not execute statement: $dbh->errstr";
	} else {
...
sub get_redes_hash {
    my ( $client_id,$ip_version,$return_int,$client_only, $keytype,$no_rootnet,$rootnet_only ) = @_;
    ...
    my $sth = $dbh->prepare("SELECT n.red_num, n.red, n.BM, n.descr, l.loc, n.vigilada, n.comentario, c.cat, n.ip_version, INET_ATON(n.red), n.rootnet, n.client_id, n.parent_network_id FROM net n, categorias_net c, locations l WHERE c.id = n.categoria AND l.id = n.loc $ip_version_expr $client_expr $no_rootnet_expr $rootnet_only_expr");
        $sth->execute() or die "Can not execute statement: $dbh->errstr";
        while ( $ip_ref = $sth->fetchrow_hashref ) {
            ...
            if ( $keytype eq "netbm" ) {
                push @{$values_redes{"$red/$BM"}},"$red","$BM","$descr","$loc","$cat","$vigilada","$comentario","$ip_version","$red_int","$rootnet","$client_id","$parent_network_id";
            } elsif ( $keytype eq "redint" ) {
                push @{$values_redes{"$red_int"}},"$red","$BM","$descr","$loc","$cat","$vigilada","$comentario","$ip_version","$red_int","$rootnet","$client_id","$parent_network_id";
            } else {
                push @{$values_redes{$red_num}},"$red","$BM","$descr","$loc","$cat","$vigilada","$comentario","$ip_version","$red_int","$rootnet","$client_id","$parent_network_id";
            }
        }
