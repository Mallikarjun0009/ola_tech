...
my $snmp_port = $daten{'snmp_port'} || 161;
...
$gip->print_error("$client_id","$$lang_vars{port_numerical_message}") if $snmp_port !~ /^\d{1,5}$/;
...
if ( $use_snmp_group && ! $snmp_group_name ) {
    $gip->print_error("$client_id","$$lang_vars{choose_snmp_group_message}");
}
if ( $use_snmp_group && $snmp_group_name ) {
...
my $snmp_info_session=$gip->create_snmp_info_session("$client_id","$node","$community","$community_type","$snmp_version","$auth_pass","$auth_proto","$auth_is_key","$priv_proto","$priv_pass","$priv_is_key","$sec_level",$mibdirs_ref,"$vars_file","$debug","$snmp_port");
...
my %if_values=();
my ($ifDescr,$ifIndex,$ifType,$ifSpeed,$ifPhysAddress,$ifAdminStatus,$ifOperStatus,$ifName,$ifAlias,$ifInErrors,$ifOutErrors,$ifLastChange,$duplex,$neighbor_ip,$neighbor_port,$duplex_admin,$vlan_num,$vlan_default,$vlan_default_name,$ifIP,$ifIPv6);
$ifDescr=$ifIndex=$ifType=$ifSpeed=$ifPhysAddress=$ifAdminStatus=$ifOperStatus=$ifName=$ifAlias=$ifInErrors=$ifOutErrors=$ifLastChange=$duplex=$neighbor_ip=$neighbor_port=$duplex_admin=$vlan_num=$vlan_default=$vlan_default_name=$ifIP=$ifIPv6="";
...
#TEST
my $i_IP=$snmp_info_session->ip_index() || {};
my $i_IPv6=$snmp_info_session->ipv6_index() || {};
#my $i_IPv6phys=$snmp_info_session->c_inet_phys_address() || ();
#print "TEST: $i_IP - $i_IPv6<br>\n";
...
my $last_ind="";
my %ni_IPv6;
foreach my $key (sort { $i_IPv6{$a} <=> $i_IPv6{$b} } keys %i_IPv6 ) {
	my $ip6_dec=$key;
	my $ind=$i_IPv6{$ip6_dec};
	$ip6_dec=~s/^2\.16\.//;
	my @dec_vals=split('\.',$ip6_dec);
	my $ip6_hex="";
	...
	$ip6_hex=~s/^://;
	$ip6_hex=ip_compress_address ($ip6_hex, 6);
	if ( $last_ind && $ind == $last_ind ) {
		$ip6_hex=$ni_IPv6{$ind} . "<br>" . $ip6_hex;
	}
	$ni_IPv6{$ind}=$ip6_hex;
	$last_ind=$ind;
#        print "TEST1: $ip6_dec => $ip6_hex - $ind<br>\n";
}
...
foreach my $iid (keys %$interfaces){
	...
	$ifOperStatus=$na_gray if $ifOperStatus eq "notPresent";
	$ifAdminStatus=$i_up_admin->{$iid} || $na_gray;
	$ifAdminStatus=$na_gray if $ifAdminStatus eq "notPresent";
...
#	$ifName=$i_name->{$iid} || $na_gray;
	...
	$ifIP=$i_IP->{$iid} || $na_gray;
	$ifIPv6=$i_IPv6->{$iid} || $na_gray;

	if ( $ifIP ne $na_gray && $ifIPv6 ne $na_gray ) {
		$ifIP=$ifIP . "<br>" . $ifIPv6;
	} elsif ( $ifIP eq $na_gray && $ifIPv6 ne $na_gray ) {
		$ifIP=$ifIPv6;
	}

	push @{$if_values{$iid}},"$ifDescr","$ifType","$ifIP","$ifSpeed","$ifPhysAddress","$ifAdminStatus","$ifOperStatus","$ifName","$ifAlias","$ifInErrors","$ifOutErrors","$ifLastChange";
...
if ( $cisco_index == 1 ) {
	...
	foreach my $v_num(keys %$vlan_numbers) {
		...
		my $snmp_info_session=$gip->create_snmp_info_session("$client_id","$node","$community_orig","$community_type","$snmp_version","$auth_pass","$auth_proto","$auth_is_key","$priv_proto","$priv_pass","$priv_is_key","$sec_level",$mibdirs_ref,"$vars_file","$debug","$snmp_port");
		if ($snmp_info_session->{ErrorStr}) {
			if ( $snmp_info_session->{ErrorStr} =~ /nosuchname/i ) {
			#               print "<p><br><b>$node</b>: $$lang_vars{nosuchname_snmp_error_message}\n";
			} elsif ( $snmp_info_session->{ErrorStr} =~ /authentication failure/i ) {
        ...
        my $fw_mac;
        eval { $fw_mac     = $snmp_info_session->fw_mac(); };
        if ( ! $@ ) {
...
my $session=$gip->create_snmp_session("$client_id","$node","$community","$community_type","$snmp_version","$auth_pass","$auth_proto","$auth_is_key","$priv_proto","$priv_pass","$priv_is_key","$sec_level","$vars_file","$snmp_port") || $gip->print_end("$client_id", "", "", "");
...
if ( %if_values ) {
	...
	foreach my $key ( sort { $a <=> $b } keys (%if_values) ) {
		...
		$ifIP=$value[0]->[2] || $na_gray;
		$ifSpeed=$value[0]->[3] || $na_gray;
		$ifPhysAddress=$value[0]->[4] || $na_gray;
		$ifAdminStatus=$value[0]->[5] || $na_gray;
		$ifOperStatus=$value[0]->[6] || $na_gray;
		$ifName=$value[0]->[7] || $na_gray;
		$ifAlias=$value[0]->[8] || $na_gray;
		$ifInErrors=$value[0]->[9] || "-";
		$ifOutErrors=$value[0]->[10] || "-";
		$ifLastChange=$value[0]->[11] || $na_gray;
		$duplex=$value[0]->[12] || $na_gray;
		$neighbor_ip=$value[0]->[13] || $na_gray;
		$neighbor_port=$value[0]->[14] || $na_gray;
		my $duplex_admin=$value[0]->[15] || $na_gray;
