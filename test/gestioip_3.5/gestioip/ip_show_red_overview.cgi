...
my $ip_version = $daten{'ip_version'} || "HOLA";
...
if ( $ip_version !~ /^(v4|v6)$/ ) {
        $client_id = 1;
#        $gip->CheckInput("$client_id",\%daten,"$$lang_vars{mal_signo_error_message}","$$lang_vars{redes_message}","$vars_file");
#	$gip->print_error("$client_id","$$lang_vars{formato_malo_message} (2) $ip_version TEST");
}
...
if ( $first_ip_int_ping && $last_ip_int_ping ) {
	...
	$ip_ad=$gip->int_to_ip("$client_id","$i","$ip_version");
			...
			$pid = $pm->start("$ip_ad") and next;
                ...
                my $result;
				if ( $ip_version eq "v4" ) {
                    $result=$gip->ping_system("$ip_ad","$ping_timeout");
				} else {
					my $command='ping6 -c 1 ' .  $ip_ad;
					$result=$gip->ping6_system("$command","0");
				}
...
my $range_address = "0";
my $count_help="";
$count_help = Math::BigInt->new("$count_help");
my $range_comentario;
	...
	if ( $ip[$k]->[0] == $ip_int_count ) {
		...
		if ( $view eq "short" ) {
			...
			if ( defined ($cat) && $hostname ne "---" && defined ($ip[$k]->[8]) && $ip[$k]->[8] eq "1" ) {
				...
				} elsif ( $cat =~ /[Ww]indows [Ss]/ ) {
					$button_class = "server_windows_ok_button";
				} elsif ( $cat =~ /[Ll]inux [Ss]/ ) {
					$button_class = "server_linux_ok_button";
				} elsif ( $cat eq "DB" ) {
					$button_class = "database_ok_button";
				} elsif ( $cat eq "LB" ) {
					$button_class = "LB_ok_button";
				} elsif ( $cat eq "AP" ) {
					$button_class = "access_point_ok_button";
				} elsif ( $cat eq "wifi" ) {
				...
				if ( $cat eq "Router" || $cat eq "L3 device" ) {
					...
					$button_class = "server_windows_failed_button";
				} elsif ( $cat =~ /[Ll]inux [Ss]/ ) {
					$button_class = "server_linux_failed_button";
				} elsif ( $cat eq "DB" ) {
					$button_class = "database_failed_button";
				} elsif ( $cat eq "LB" ) {
					$button_class = "LB_failed_button";
				} elsif ( $cat eq "AP" ) {
					$button_class = "access_point_failed_button";
				} elsif ( $cat eq "wifi" ) {
				...
				if ( $cat eq "Router" || $cat eq "L3 device" ) {
					...
					$button_class = "server_windows_never_checked_button";
				} elsif ( $cat =~ /[Ll]inux [Ss]/ ) {
					$button_class = "server_linux_never_checked_button";
				} elsif ( $cat eq "DB" ) {
					$button_class = "database_never_checked_button";
				} elsif ( $cat eq "LB" ) {
					$button_class = "LB_never_checked_button";
				} elsif ( $cat eq "AP" ) {
					$button_class = "access_point_never_checked_button";
				} elsif ( $cat eq "wifi" ) {
...
#NET32
		...
		if ( $view eq "short" ) {
			if ( defined($hostname) ) {
				print "<td align=\"center\" valign=\"middle\" class=\"$button_class\" onClick=\"checkhost(\'$ip_show\',\'$hostname\',\'$client_id\',\'$ip_version\',\'$red_num\')\" title=\"$ip_title\"><span class=\"overview_table_text\">$fourth_oct</span></td>\n";
			} else {
				print "<td align=\"center\" valign=\"middle\" class=\"$button_class\" onClick=\"checkhost(\'$ip_show\',\'\',\'$client_id\',\'$ip_version\',\'$red_num\')\" title=\"$ip_title\"><span class=\"overview_table_text\">$fourth_oct</span></td>\n";
			}
		} else {
...
if ( $view eq "short" ) {
	...
	print "<tr><td><img src=\"$server_proto://$base_uri/imagenes/server.png\" alt=\"server\"></td><td>server</td><td><img src=\"$server_proto://$base_uri/imagenes/server_windows.png\" alt=\"Windows Server\"></td><td>Windows Server</td><td><img src=\"$server_proto://$base_uri/imagenes/server_linux.png\" alt=\"Linux Server\"></td><td>Linux Server</td></tr>\n";

	print "<tr><td><img src=\"$server_proto://$base_uri/imagenes/LB.png\" alt=\"Load Balancer\"></td><td>Load Balancer</td><td><img src=\"$server_proto://$base_uri/imagenes/database.png\" alt=\"DB\"></td><td>DB ($$lang_vars{database_message})</td><td><img src=\"$server_proto://$base_uri/imagenes/workstation.png\" alt=\"workst\"></td><td>workstation</td></tr>\n";

	print "<tr><td><img src=\"$server_proto://$base_uri/imagenes/mobil_equipo.png\" alt=\"wifi\"></td><td>wifi</td><td><img src=\"$server_proto://$base_uri/imagenes/access_point.png\" alt=\"Access Point\"></td><td>Access Point</td><td><img src=\"$server_proto://$base_uri/imagenes/phone.png\" alt=\"VoIP\"></td><td>VoIP</td></tr>";

	print "<tr><td><img src=\"$server_proto://$base_uri/imagenes/printer.png\" alt=\"printer\"></td><td>printer</td><td><img src=\"$server_proto://$base_uri/imagenes/other.png\" alt=\"other\"></td><td>other</td><td><img src=\"$server_proto://$base_uri/imagenes/unknown_never_checked_show.png\" alt=\"never checked\"></td><td>$$lang_vars{desconocido_message}</td></tr>";
