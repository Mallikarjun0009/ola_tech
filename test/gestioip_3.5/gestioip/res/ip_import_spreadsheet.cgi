...
my %dns_server_groups = $gip->get_dns_server_group_hash("$client_id","name");
my @snmp_groups = $gip->get_snmp_groups("$client_id");
my @dns_zones = $gip->get_dns_zones("$client_id","","","A");
my @dns_aaaa_zones = $gip->get_dns_zones("$client_id","","","AAAA");
push(@dns_zones, @dns_aaaa_zones);
my @dns_ptr_zones = $gip->get_dns_zones("$client_id","","","PTR");
my %scan_a_zone_hash;
foreach my $z (@dns_zones ) {
    push @{$scan_a_zone_hash{$z->[1]}},"$z->[0]","$z->[3]";
}
my @vlans = $gip->get_vlans("$client_id","","","PTR");
my %vlan_hash;
$k = 0;
foreach ( @vlans ) {
    my $vlan_id = $vlans[$k]->[0] || "";
    my $vlan_num = $vlans[$k]->[1] || "";
    my $vlan_name = $vlans[$k]->[2] || "";
    my $vlan_key = $vlan_num . " - " . $vlan_name;
    $vlan_hash{$vlan_key}=$vlan_id if $vlan_id && $vlan_num && $vlan_name;
    $k++;
}
	...
	if (( $sheet->{Name} eq "$excel_sheet_name" && $daten{'hoja'} && $daten{'sheet_import_type'} eq "one_sheet" ) || ( $daten{'sheet_import_type'} eq "all_sheet" && ! $daten{'hoja'} && ! $daten{'some_sheet_values'} ) || ( $daten{'sheet_import_type'} eq "some_sheet" && $j =~ /^($import_sheet_numbers)$/ )) {
		...
		foreach my $row ($sheet->{MinRow} .. $sheet->{MaxRow}) {
			...
			if ( $ip_version eq "v4" ) {
				...
				if ( $entries{redes} =~ /^\d{1,3}\.\d{1,3}\.\d{1,3}\.00\d$/ ) {
                    $entries{redes} =~ s/00(\d)$/$1/;
				} elsif ( $entries{redes} =~ /^\d{1,3}\.\d{1,3}\.\d{1,3}\.0\d\d$/ ) {
                    $entries{redes} =~ s/0(\d\d)$/$1/;
                }
...
#                my $utf8 = encode("gb2312-raw", $entries{$key});
#                my $utf8_n = decode("euc-cn", $utf8);
                my $utf8 = encode("euc-cn", $entries{$key});
                my $utf8_n = decode("gb2312-raw", $utf8);
...
#			print "<b>$entries{redes}/$entries{BM}</b>  ";
			...
			$red_num = $gip->insert_net("$client_id","$entries{redes}","$entries{BM}","$entries{descr}","$loc_id","$sync","$entries{comentario}","$cat_id","$ip_version","$rootnet_val");
			...
			for ( my $k = 0; $k < scalar(@cc_values); $k++ ) {
				if ( $entries{"$cc_values[$k]->[0]"} ) {
                    ...
                    if ( $cc_name eq "Tag" ) {
						...
						if ( @new_tags ) {
							if ( @object_tags ) {
								$gip->delete_tag_entry_object_obj_id("$client_id", "$red_num", "network");
							}

							foreach ( @new_tags ) {
								my $tag_id = $tag_hash{$_}[0];
								$gip->insert_tag_for_object("$client_id", "$tag_id", "$red_num", "network");
							}
						}
						...
						my @new_z;
                        foreach ( @z ) {
                            next if ! $scan_a_zone_hash{$_};
							push @new_z, "$_";
                        }

                        my @objects = $gip->get_network_scan_zones("$client_id", "$red_num", "A");
                        my @objects_aaaa = $gip->get_network_scan_zones("$client_id", "$red_num", "AAAA");
                        push(@objects, @objects_aaaa);


						if ( @new_z ) {
							if ( @objects ) {
                                my $zone_id = $objects[0]->[0];
								$gip->delete_scan_zone_from_net("$client_id", "$zone_id", "$red_num");
							}

							foreach ( @new_z ) {
								my $zone_id = $scan_a_zone_hash{$_}[0];
								$gip->insert_scan_zone_for_object("$client_id", "$zone_id", "$red_num");
							}
						}
                        ...
                        # check if DNSSG exists
                        $cc_value =~ s/\s*//g;

						next if ! $dns_server_groups{$cc_value};

						$cc_value = $dns_server_groups{$cc_value}[0] || "";
						...
						my $cc_value_found = 0;
						foreach ( @snmp_groups ) {
							my $snmp_group_name = $snmp_groups[$i]->[1];
							$cc_value_found = 1 if $snmp_group_name eq $cc_value;
							$i++;
						}

						next if ! $cc_value_found;
						...
						my $cc_value_found = 0;
						foreach ( @dns_zones ) {
							my $dns_zone_name = $dns_zones[$i]->[1];
							$cc_value_found = 1 if $dns_zone_name eq $cc_value;
							$i++;
						}
						...
						my $i = 0;
						my $cc_value_found = 0;
						foreach ( @dns_ptr_zones ) {
							my $dns_zone_name = $dns_ptr_zones[$i]->[1];
							$cc_value_found = 1 if $dns_zone_name eq $cc_value;
							$i++;
						}
            ...
            $gip->update_net_usage_cc_column("$client_id", "$ip_version", "$red_num","$entries{BM}", "no_rootnet");
		}
	}
	$j++;
}
