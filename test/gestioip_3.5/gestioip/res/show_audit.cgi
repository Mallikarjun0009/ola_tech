...
if ( $time_radio eq "time_range" ) { 
	...
	$audit_audit_time_range = "last $time_range";
	...
	$audit_audit_time_range = "$start_date_form - $end_date_form";
	...
	$audit_audit_time_range = "last 3 weeks";
...
# INSERT AUDIT
my $audit_type_audit="171";
my $audit_class_audit="3";
my $update_type_audit_audit="1";
my $event_more = "filter: $audit_audit_time_range; $search; $update_type_audit; $event_class; $event";
$event_more =~ s/NULL/---/g;

$gip->insert_audit("$client_id","$audit_class_audit","$audit_type_audit","$event_more","$update_type_audit_audit","$vars_file");
...
if ( $values_audit[0] ) {
	...
	foreach (@values_audit) {
		...
		my $remote_host=$values_audit[$k]->[7] || "N/A";
		$client_name="---" if $client_name eq "9999";
		my $client_name_td="";
		$client_name_td="<td nowrap>$client_name&nbsp;&nbsp;&nbsp;</td>" if $all_clients eq "yy";
		print "<tr bgcolor=\"$color\">$client_name_td<td nowrap>$date&nbsp;&nbsp;&nbsp;</td><td nowrap>$user&nbsp;&nbsp;&nbsp;</td><td nowrap>$remote_host&nbsp;&nbsp;&nbsp;</td><td nowrap>$event_type&nbsp;&nbsp;&nbsp;</td><td nowrap>$event_class&nbsp;</td><td nowrap>&nbsp;$event&nbsp;</td><td>$rtl_helper${event_value}${rtl_helper}</td></tr>\n";
		$k++;
	}
	print "</table>\n";
} else {
