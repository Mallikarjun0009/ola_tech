...
print "<tr><td $align1><input type=\"radio\" name=\"IP_format\" id=\"IP_format2\" value=\"last_oct\" onclick=\"network_field.disabled=false;\"> $$lang_vars{import_hosts_only_last_oct_message} <input type=\"text\" class='form-control form-control-sm' style='width: 12em' name=\"network_field\" size=\"5\" maxlength=\"5\" disabled> $$lang_vars{network_address_field_message}$rtl_helper</td></tr>\n";
...
print "<tr><td $align><select class='custom-select custom-select-sm' style='width: 4em' name=\"ip\" size=\"1\">";
...
print "<tr><td $align><select class='custom-select custom-select-sm' style='width: 4em' name=\"hostname\" size=\"1\">";
foreach (@nums) {
	print "<option>$_</option>\n";
}
print "</select>\n";
...
for ( my $k = 0; $k < scalar(@cc_values); $k++ ) {
	...
	next if $cc_name eq "VLAN";
	next if $cc_name eq "Sec_Zone";
...
print "<td $align1><br><select class='custom-select custom-select-sm' style='width: 4em' name=\"update_type\" size=\"1\">";
print "<option></option>";
my $j=0;
