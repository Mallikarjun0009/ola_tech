...
if ( ! -e $excel_file_name ) {
	$gip->print_error("$client_id","$$lang_vars{no_spreadsheet_message} \"$spreadsheet\"<p>$$lang_vars{no_spreadsheet_message} \"$spreadsheet\"");
}
...
my $update_comment = $daten{update_comment} || "";
...
#TEST $firstfound
			...
			my $vlan_exists=0;
			foreach ( %vlans ) {
				if ( exists($vlans{"${vlan_id}-${vlan_name}"}) ) {
					print "$$lang_vars{vlan_exists_message} - $$lang_vars{ignorado_message}<br>\n";
                    my $db_id = $vlans{"${vlan_id}-${vlan_name}"};
                    my @vlan_found = $gip->get_vlan("$client_id", "$db_id");
                    my $db_descr = "$vlan_found[0]->[2]" || "";
                    my $db_bg_color = "$vlan_found[0]->[3]" || "";
                    my $db_font_color = "$vlan_found[0]->[4]" || "";
                    my $db_provider_id = "$vlan_found[0]->[5]" || "";

					$vlan_exists=1;

                    if ( $update_comment eq "add" ) {
                        if ( $db_descr !~ /$vlan_descr/ && $vlan_descr ne "NULL" ) {
                            my $new_descr = $db_descr . " - " . $vlan_descr;
                            $gip->update_vlan("$client_id","$db_id","$vlan_id","$vlan_name","$db_provider_id","$new_descr","$db_bg_color","$db_font_color");
                        }    
                    } elsif ( $update_comment eq "replace" && $vlan_descr ne "NULL" ) {
                        $gip->update_vlan("$client_id","$db_id","$vlan_id","$vlan_name","$db_provider_id","$vlan_descr","$db_bg_color","$db_font_color");
                    }    
					last;
				}
			}
...
if ( $vlan_found == "1" ) {
	$gip->print_error("$client_id","$$lang_vars{no_vlans_sp_message}");
}
