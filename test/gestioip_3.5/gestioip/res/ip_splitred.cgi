...
for (my $l=0; $l <= $new_redes_count; $l++) {
	...
	$new_redes_detail[$l]->[7] = $ip_version;
}
...
if ( $user_management_enabled eq "yes" ) {
	my $loc_old=$gip->get_loc_from_redid("$client_id","$red_id_old");
	my $loc_id_old=$gip->get_loc_id("$client_id","$loc_old") || "-1";
    $gip->check_loc_perm_rw("$client_id","$vars_file", "$locs_rw_perm", "$loc_old", "$loc_id_old");
}
...
#	$gip->insert_net("$client_id","$red_id_nuevo","$red_nuevo","$BM_nuevo","$descr_nuevo","$loc_id_nuevo","$vigilada_nuevo","$comentario_nuevo","$cat_id_nuevo","$ip_version") or die $gip->print_error("$client_id","$$lang_vars{insert_net_error}");
	my $red_id_nuevo = $gip->insert_net("$client_id","$red_nuevo","$BM_nuevo","$descr_nuevo","$loc_id_nuevo","$vigilada_nuevo","$comentario_nuevo","$cat_id_nuevo","$ip_version") or die $gip->print_error("$client_id","$$lang_vars{insert_net_error}");
...
#	}
	...
	if ( $keep_hosts eq "y" && $BM_nuevo >= $SMALLEST_BM && $ip_version eq "v4" ) {
		...
		foreach my $key ( keys %$ip_hash ) { 
			...
			$gip->delete_ip("$client_id","$first_ip_int","$last_ip_int","$ip_version", "$red_id_old");
		}
	}
    ...
    $gip->update_net_usage_cc_column("$client_id", "$ip_version", "$red_id_nuevo", "$BM_nuevo","no_rootnet");
}
...
my $rootnet_old = "$values_redes[0]->[9]" || 0;
$gip->delete_red("$client_id","$red_id_old","$rootnet_old");
...
my $pages_links=$gip->get_pages_links_red("$client_id","$vars_file","$start_entry","$anz_values_redes","$entries_per_page","$tipo_ele","$loc_ele","red_auf","","","$show_rootnet","$show_endnet","$hide_not_rooted");
