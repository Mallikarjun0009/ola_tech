...
print "<td><select name=\"reserve_start_address\" class='custom-select custom-select-sm m-2' style='width: 10em' size=\"5\">";
foreach (@all_ip) {
...
print "<td><select name=\"reserve_end_address\" class='custom-select custom-select-sm m-2' style='width: 10em' size=\"5\">";
foreach (@all_ip) {
	print "<option>$_</option>\n";
}
print "</select></td><td width=\"17%\"></td></tr>";
print "<tr><td align=\"right\"><p><br>$$lang_vars{reservar_comment_message}</td>";
print "<td colspan=\"3\"><p><br><input type='text' class='form-control form-control-sm m-2' style='width: 12em' name=\"comentario\" size=\"20\" maxlength=\"30\"></td></tr>\n";
print "<tr><td align=\"right\">$$lang_vars{tipo_rango_message}</td>";
print "<td><select name=\"range_type_id\" class='custom-select custom-select-sm m-2' style='width: 10em' size=\"3\">";
$j=0;
...
print "</select></td>";
print "<tr><td align=\"right\">$$lang_vars{preserve_hosts_message}</td><td><input type=\"checkbox\" name=\"preserve_hosts\" class='m-2' value=\"1\" checked></td></tr>";
...
if ( @rangos ) {
	...
	if ( $rangos_red[0] ) {
		...
		print "<select name=\"range_id\" class='custom-select custom-select-sm' size=\"1\">";
		foreach ( @rangos_red ) {
			print "$_";
		}
        ...
        print "<tr><td colspan=\"2\" >$$lang_vars{preserve_hosts_message}<input type=\"checkbox\" name=\"preserve_hosts\" class='m-2' value=\"1\" checked></td></tr>";
		print "<tr><td colspan=\"2\"><br><input name=\"order_by\" type=\"hidden\" value=\"$order_by\"><input name=\"start_entry_hosts\" type=\"hidden\" value=\"$start_entry_hosts\"><input name=\"entries_per_page_hosts\" type=\"hidden\" value=\"$entries_per_page_hosts\"><input name=\"client_id\" type=\"hidden\" value=\"$client_id\"><input name=\"ip_version\" type=\"hidden\" value=\"$ip_version\">$text_field_number_given_form<input type=\"submit\" value=\"$$lang_vars{borrar_message}\" name=\"B2\" class=\"btn\" $onclick></form></td></tr>\n";
	} else {
