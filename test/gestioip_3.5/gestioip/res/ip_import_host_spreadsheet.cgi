...
#### TEST /^(\w*.......... ueberarbeiten....
		...
		foreach my $row ($sheet->{MinRow} .. $sheet->{MaxRow}) {
			...
			if ( $firstfound == 1 && $ip_int >= $first_ip_int && $ip_int <= $last_ip_int) {
				if ( ( $ip_int == "$first_ip_int" && $ip_version eq "v6" ) || ( $ip_int == "$first_ip_int" &&  $ip_version eq "v4" && $host_red_bm != 31 && $host_red_bm != 32 ) ) {
					if ( $vars_file =~ /vars_he$/ ) {
					...
					$to_ignore="1";
				} elsif (( $ip_int == "$last_ip_int" && $ip_version eq "v6" ) || ( $ip_int == "$last_ip_int" &&  $ip_version eq "v4" && $host_red_bm != 31 && $host_red_bm != 32 )) {
					if ( $vars_file =~ /vars_he$/ ) {
...
#					print "<b>$entries{ip}</b>: $entries{hostname} ";
            ...
            if ( $net_found == 0 && $ip_version eq "v6" ) {
                print "<b>$entries{ip}</b>: $$lang_vars{import_no_red_message} - $$lang_vars{ignorado_message}<br>\n";
                next;
            }
			$firstfound = 1;
            ...
            for ( my $k = 0; $k < scalar(@cc_values); $k++ ) {
                if ( $entries{"$cc_values[$k]->[0]"} ) {
					...
					if ( $cc_name eq "Tag" ) {
						...
						if ( @new_tags ) {
							if ( @object_tags ) {
								$gip->delete_tag_entry_object_obj_id("$client_id", "$host_id", "host");
							}

							foreach ( @new_tags ) {
								my $tag_id = $tag_hash{$_}[0];
								$gip->insert_tag_for_object("$client_id", "$tag_id", "$host_id", "host");
...
foreach my $rn ( @redes_usage_array) {
    $gip->update_net_usage_cc_column("$client_id", "$ip_version", "$rn","", "no_rootnet");
}
