...
my $return = $gip->insert_mac("$client_id","$mac","$duid","$account","$host","$comment");
if ( $return ) {
        $gip->print_error("$client_id","$$lang_vars{mac_exists_message}")
}
delete @ENV{'PATH', 'IFS', 'CDPATH', 'ENV', 'BASH_ENV'};
my ($exit, $error) = $gip->update_mac_extern("$client_id","$mac","insert");
...
my $event="$mac,$duid,$account,$host,$comment";
