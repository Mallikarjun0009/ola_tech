...
foreach $network_cidr_mask(@route_dests_cidr) {
	...
	if ( $add_comment eq "y" ) {
		$red_num = $gip->insert_net("$client_id","$network","$BM",'','-1',"$sync","$comment",'-1',"$ip_version");
	} else {
		$red_num = $gip->insert_net("$client_id","$network","$BM",'','-1',"$sync",'','-1',"$ip_version");
	}
...
#	} 
	...
	if ( $add_comment eq "y" ) {
		$red_num = $gip->insert_net("$client_id","$network","$BM",'','-1',"$sync","$comment",'-1',"$ip_version","$rootnet_val");
	} else {
		$red_num = $gip->insert_net("$client_id","$network","$BM",'','-1',"$sync",'','-1',"$ip_version","$rootnet_val");
	}
...
if ( $import_ipv4 eq "ipv4" && ! $route_dests_cidr[0] ) {
	print "<p><span style=\"float: $ori\">${rtl_helper}$$lang_vars{no_matching_networks_v4_found_message}</span><br>\n";
}
if (  $import_ipv6 eq "ipv6" && ! $route_dests_cidr_ipv6[0] ) {
	print "<p><span style=\"float: $ori\">${rtl_helper}$$lang_vars{no_matching_networks_v6_found_message}</span><br>\n";
}
