...
function check_import_type(TYPE) {
        ...
        if ( TYPE == 'networks' ) {
		  ...
		  document.send_spreadsheet.action = "$server_proto://$base_uri/res/ip_import_site_spreadsheet_form1.cgi";
        } else if ( TYPE == 'lines' ) {
		  document.send_spreadsheet.action = "$server_proto://$base_uri/res/ip_import_line_spreadsheet_form1.cgi";
        }
}
-->
</script>
...
$$lang_vars{redes1_message}<input type="radio" name="import_type" value="networks" onchange="check_import_type('networks');" checked>&nbsp;&nbsp;&nbsp;&nbsp;$$lang_vars{hosts1_message}<input type="radio" name="import_type" value="hosts" onchange="check_import_type('hosts');">&nbsp;&nbsp;&nbsp;&nbsp;$$lang_vars{vlans_message}<input type="radio" name="import_type" value="vlans" onchange="check_import_type('vlans');">&nbsp;&nbsp;&nbsp;&nbsp;$$lang_vars{locs_message}<input type="radio" name="import_type" value="sites" onchange="check_import_type('sites');">&nbsp;&nbsp;&nbsp;&nbsp;$$lang_vars{LL_message}<input type="radio" name="import_type" value="lines" onchange="check_import_type('lines');"> 
<font color=\"white\">x</font>
</td></tr>
<tr><td $align1>
<form id="send_spreadsheet" name="send_spreadsheet" action="$server_proto://$base_uri/res/ip_import_spreadsheet_form1.cgi" method="post" enctype="multipart/form-data" autocomplete="off">
<p><input type="hidden" name="client_id" value="$client_id"><input type="file" name="spreadsheet" class="btn" style="margin: 1em;"></p>
</td></tr>
<tr><td $align1><p><br>
