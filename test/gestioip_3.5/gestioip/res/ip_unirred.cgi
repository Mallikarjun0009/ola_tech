...
if ( $overlap_found[0] ) {
	...
	foreach ( @overlap_found ) {
		...
		my $loc_id_old=$gip->get_loc_id("$client_id","$loc_old_audit") || "-1";
		...
		if ( $user_management_enabled eq "yes" ) {
			$gip->check_loc_perm_rw("$client_id","$vars_file", "$locs_rw_perm", "$loc_old_audit", "$loc_id_old");
		}
	...
	$red_id_nuevo = $gip->insert_net("$client_id","$red_nuevo","$BM_nuevo","$descr","$loc_id_nuevo","$vigilada","$comentario","$cat_net_id_nuevo","$ip_version") or die $gip->print_error("$client_id",$$lang_vars{algo_malo_message});
...
my $div_notify = GipTemplate::create_div_notify_text(
    noti => "$$lang_vars{agregado_message}: $new_range",
);
print "$div_notify\n";
...
my $pages_links=$gip->get_pages_links_red("$client_id","$vars_file","$start_entry","$anz_values_redes","$entries_per_page","","","red_auf","","","$show_rootnet","$show_endnet","$hide_not_rooted");
