...
my $script_base_path = '/usr/share/gestioip';
my $bin_path = $script_base_path . '/bin';
my $gcloud_auth_path = $script_base_path . '/etc/auth/gcloud';
...
$gip->CheckInput("$client_id",\%daten,"$$lang_vars{mal_signo_error_message}","$$lang_vars{jobs_message}","$vars_file");

foreach my $d ( keys %daten ) {
	my $error_message = "";
	if ( $d =~ /^(mail_recipients|mail_from)$/ ) {
		$error_message=$gip->check_parameters( email_list => "$daten{$d}" );
	} elsif ( $d =~ /^(start_date|end_date)$/ ) {
		$error_message=$gip->check_parameters( date => "$daten{$d}" );
	} elsif ( $d =~ /^(CSV_nodes|CSV_networks)$/ ) {
		$error_message=$gip->check_parameters( network_with_bm => "$daten{$d}" );
	} elsif ( $d =~ /^(kea_url)$/ ) {
		$error_message=$gip->check_parameters( url => "$daten{$d}" );
	} elsif ( $d =~ /^(azure_secret_key_value)$/ ) {
		$error_message=$gip->check_parameters( azure_secret_key => "$daten{$d}" );
	} elsif ( $d =~ /^(aws_secret_access_key)$/ ) {
		$error_message=$gip->check_parameters( aws_secret_access_key => "$daten{$d}" );
	} elsif ( $d !~ /^(name|discovery_type|status|run_once|execution_time|comment|send_result_by_mail|verbose|debug|client_id|B2|execution_time)$/ ) {
		$error_message=$gip->check_parameters( scheduled_job_param => "$daten{$d}" );
	}
	$gip->print_error("1","$error_message" ) if $error_message;
}
...
if ( $run_once ) {
    $gip->print_error("$client_id","$$lang_vars{execution_time_message}: $$lang_vars{formato_malo_message}") if $execution_time !~ /^(\d+)\/(\d+)\/(\d+) (\d+):(\d+)/;
...
$gip->print_error("$client_id","$$lang_vars{job_name_exists_message}") if $job_hash->{$name};
...
} elsif ( $discovery_type == 8 ) {
    $script = "gip_cloud_sync.pl";
} elsif ( $discovery_type == 9 ) {
    $script = "gip_cloud_sync.pl";
} elsif ( $discovery_type == 10 ) {
    $script = "gip_cloud_sync.pl";
} elsif ( $discovery_type == 11 ) {
...
if ( $discovery_type == 1 ) {
#    $script_options .= " --execute_network_discovery";
...
my ($CSV_nodes, $use_tags, $nodes, @nodes);
...
if ( $discovery_type == 1 || $discovery_type == 2 || $discovery_type == 5 ) {
    ...
    if ( $write_to_file ) {
        $gip->print_error("$client_id","$$lang_vars{check_file_name_message}") if $write_to_file !~ /^[A-Za-z0-9-_]+$/;
...
my ($CSV_networks, $networks_file, $use_range, $location_scan, $networks, @networks);
...
if ( $discovery_type == 3 || $discovery_type == 4 ) {
	$gip->print_error("$client_id","$$lang_vars{choose_networks_job_message}") if ! $networks && ! $use_tags && ! $use_range && ! $location_scan;
	$gip->print_error("$client_id","$$lang_vars{choose_ip_version_message}") if ! $actualize_ipv4 && ! $actualize_ipv6;
}
...
if ( $discovery_type == 10 ) {
    ...
    my $gcp_key_file_full = $gcloud_auth_path . "/" . $gcp_key_file;
    if ( ! -e $gcp_key_file_full ) {
        $gip->print_error("$client_id","$$lang_vars{key_file_not_found_message}: $gcp_key_file_full");
    }
    if ( ! -r $gcp_key_file_full ) {
        $gip->print_error("$client_id","$$lang_vars{key_file_not_readable_message}: $gcp_key_file_full");
    }
...
my $job_id = $gip->insert_scheduled_job(
    client_id=>"$client_id",
    name=>"$name",
    type=>"$discovery_type",
    status=>"$status",
...
$script_options .= " --W=\"$job_id\"";

$gip->update_scheduled_job_arguments("$client_id", "$job_id", "$script_options");

my $cron_entry;
if ( $debug ) {
    $cron_entry = $cron_time . " " . $script . " " . $script_options . " >/tmp/"  . $name . "_out.txt 2>&1 #ID: " . $job_id;
} else {
    $cron_entry = $cron_time . " " . $script . " " . $script_options . " >/dev/null 2>&1 #ID: " . $job_id;
}

my $error = $gip->create_cron_entry("$cron_entry");

if ( $error ) {
    $gip->delete_scheduled_job("$client_id","$job_id");
    if ( $error =~ /because of pam configuration/ ) {
		...
		$gip->print_error("$client_id","$$lang_vars{create_job_error_message}<br>$error<p>");
    }   
}
...
print <<EOF;
<script>
update_nav_text("$$lang_vars{job_added_message}");
</script>
EOF

$gip->PrintJobTab("$client_id", "$vars_file", "$job_id");
