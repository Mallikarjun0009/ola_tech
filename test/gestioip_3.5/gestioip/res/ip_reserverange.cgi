...
my $preserve_hosts=$daten{'preserve_hosts'} || 0;
...
$gip->print_error("$client_id","$$lang_vars{formato_malo_message} (4)") if $preserve_hosts !~ /^\d$/;
	...
	for ($i = $reserve_start_address_int; $i <= $reserve_end_address_int; $i++) {
		if ( $ch[$j]) {
			if ( $ch[$j]->[0] eq "$i" ) {
                ...
                if ( $preserve_hosts ) {
                    my $loc_p = $ch[$j]->[3] || "";
                    $loc_p = "" if $loc_p eq "NULL";
                    my $cat_p = $ch[$j]->[4] || "";
                    $cat_p = "" if $cat_p eq "NULL";

                    my $loc_id_sub = "";
                    my $cat_id_sub = "";
                    $loc_id_sub = $red_loc_id if ! $cat_p;
                    $cat_id_sub = $cat_id if ! $cat_p;

                    $gip->update_range_id_host_id_only("$client_id","$new_range_id","$i","$loc_id_sub", "$cat_id_sub");
...
#	print "<p><b>$$lang_vars{rango_added_message}: <i>$reserve_start_address-$reserve_end_address ($comentario)</i></b><br>\n";
	...
	my $div_notify = GipTemplate::create_div_notify_text(
		noti => "$$lang_vars{rango_added_message}: $reserve_start_address-$reserve_end_address ($comentario)",
	);
	print "$div_notify\n";
	...
	$gip->delete_range("$client_id","$range_id","$preserve_hosts");

	my $div_notify = GipTemplate::create_div_notify_text(
		noti => "$$lang_vars{rango_borrado_message}: $start_ip_audit-$end_ip_audit ($rangos[0]->[2])",
	);
	print "$div_notify\n";
