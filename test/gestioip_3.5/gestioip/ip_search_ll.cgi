...
if ( ! $daten{'line'} && ! $daten{'type'} && ! $daten{'service'} && ! $daten{'description'} && ! $daten{'phone_number'} && ! $daten{'ad_number'} && ! $daten{'loc_id'} && ! $daten{'room'} && ! $daten{'device'} && ! $daten{'comment'} && ! $cc_value_exists && ! $ll_id && ! $daten{'ll_client_id'}) {
	$gip->print_error("$client_id","$$lang_vars{no_search_string_message}");
}

if ( $ll_id ) {
    $gip->PrintLLTab("$client_id","$vars_file","","","","$ll_id");
} else {
    $gip->PrintLLTab("$client_id","$vars_file","",\%daten,"","");
}
