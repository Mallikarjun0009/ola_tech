...
my $lang = $daten{'lang'} || "en";
my $from_login = $daten{'from_login'} || "";
my $login = $daten{'login'} || "";
...
#	if ( ! $ENV{HTTP_REFERER} ) {
		# direct call: change client ID if default client is not allowed by authorization
#		my $user=$ENV{'REMOTE_USER'};
#        my $id=$gip->get_user_id("$client_id","$user") || "";
#        if ( ! $id ) {
#            my $error_message = "$$lang_vars{access_denied_message}<p>$$lang_vars{user_not_found_message}";
#            $gip->print_error_with_head(title=>"$$lang_vars{gestioip_message}",headline=>"$$lang_vars{redes_dispo_message}",notification=>"$error_message",vars_file=>"$vars_file",client_id=>"$client_id");
#        }
...
#    print STDERR "TEST: CLIENT PERM: $clients_perm<br>\n";
#
...
if ( $ip[0] ) {
	$gip->PrintRedTabHead("$client_id","$vars_file","$start_entry","$entries_per_page","$pages_links","$tipo_ele","$loc_ele","$ip_version_ele","$show_rootnet","$show_endnet","$hide_not_rooted","$show_fav","$local_filter_enabled");
	...
	if ( $ip_version_ele eq "v4" ) {
		$gip->PrintRedTabHead("$client_id","$vars_file","$start_entry","$entries_per_page","$pages_links","$tipo_ele","$loc_ele","$ip_version_ele","$show_rootnet","$show_endnet","$hide_not_rooted","$show_fav","$local_filter_enabled");
		print "<br><h6>$$lang_vars{no_networks_message}</h6><br>\n";
	} else {
	...
	if ( $first_call eq "yes" ) {
		print "<p><br><h2>$$lang_vars{welcome_message}</h2><br><p>\n";
		print "<p><br><h4>$$lang_vars{first_call1_message}:</h4><p><br>\n";
		print "<h5>$$lang_vars{import_sheet_message}</h5><br>\n";
		print "<form name=\"import_red_spread\" method=\"POST\" action=\"$server_proto://$base_uri/res/ip_import_spreadsheet_form.cgi\" style=\"display:inline\"><input type=\"hidden\" name=\"client_id\" value=\"$client_id\"><input type=\"submit\" class=\"btn\" style='font-weight: bold;' value=\"$$lang_vars{import_networks_from_spreadsheet_message}\" name=\"B1\"></form><br><p><br>\n";
		print "<h5>$$lang_vars{start_initialization_message}</h5><br>\n";
		print "<form name=\"initialize\" method=\"POST\" action=\"$server_proto://$base_uri/res/ip_initialize_form.cgi\" style=\"display:inline\"><input type=\"hidden\" name=\"client_id\" value=\"$client_id\"><input type=\"submit\" class=\"btn\" style='font-weight: bold;' value=\"$$lang_vars{discover_message}\" name=\"B1\"></form><br><p><br>\n";
		print "<h5>$$lang_vars{start_network_snmp_discovery_message}</h5><br>\n";
		print "<form name=\"import_snmp\" method=\"POST\" action=\"$server_proto://$base_uri/res/ip_import_snmp_form.cgi\" style=\"display:inline\"><input type=\"hidden\" name=\"client_id\" value=\"$client_id\"><input type=\"submit\" class=\"btn\" style='font-weight: bold;' value=\"$$lang_vars{import_networks_from_snmp_message}\" name=\"B1\"></form><br><p><br>\n";
		print "<h5>$$lang_vars{add_net_manually_message}</h5><br>\n";
		print "<form name=\"insertred\" method=\"POST\" action=\"$server_proto://$base_uri/res/ip_insertred_form.cgi\"><input type=\"hidden\" name=\"client_id\" value=\"$client_id\"><input type=\"submit\" class=\"btn\" style='font-weight: bold;' value=\"$$lang_vars{nuevo_message}\" name=\"B1\"></form><br>\n";
	}
}
